...
def BusinessUpdate(request):
    ...
    if isExistsEmployee(e_name) == True and Business.objects.filter(business_code=b_code).exists() == True:
        Employee.objects.filter(user_name =e_name).update(business_code=b_code)
        return render(request, 'user/EmployeeHome.html')
